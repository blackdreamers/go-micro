module github.com/blackdreamers/go-micro/plugins/wrapper/ratelimiter/ratelimit/v3

go 1.16

require (
	github.com/blackdreamers/go-micro/plugins/broker/memory/v3 v3.0.0-20210630062103-c13bb07171bc
	github.com/blackdreamers/go-micro/plugins/registry/memory/v3 v3.0.0-20210702160030-3289c0651e5b
	github.com/blackdreamers/go-micro/plugins/transport/memory/v3 v3.0.0-20210630062103-c13bb07171bc
	github.com/blackdreamers/go-micro/v3 v3.5.2-0.20210702144234-4d243e509e39
	github.com/juju/ratelimit v1.0.1
)

replace (
	github.com/blackdreamers/go-micro/plugins/broker/memory/v3 => ../../../../plugins/broker/memory
	github.com/blackdreamers/go-micro/plugins/registry/memory/v3 => ../../../../plugins/registry/memory
	github.com/blackdreamers/go-micro/plugins/transport/memory/v3 => ../../../../plugins/transport/memory
	github.com/blackdreamers/go-micro/v3 => ../../../../../go-micro
)
